import React from 'react'

function Skils() {
    return (
        <div className='flex flex-wrap items-stretch justify-center py-10 '>
            <div className="item border p-5 flex flex-col items-center rounded-3xl  mx-2 mt-5">
                <div className="title text-text-color font-bold text-center text-3xl mb-5">
                    Javascript
                </div>
                <div className="img w-56">
                    <img className='w-full h-full max-w-full max-h-full' src="data:image/png;base64," alt="" />

                </div>
            </div>
            <div className="item border p-5 flex flex-col items-center rounded-3xl  mx-2 mt-5">
                <div className="title text-text-color font-bold text-center text-3xl mb-5">
                    HTML5
                </div>
                <div className="img w-56">
                    <img className='w-full h-full max-w-full max-h-full' src="https://img.freepik.com/free-icon/html5_318-903450.jpg" alt="" />

                </div>
            </div>
            <div className="item border p-5 flex flex-col items-center rounded-3xl  mx-2 mt-5">
                <div className="title text-text-color font-bold text-center text-3xl mb-5">
                    CSS3
                </div>
                <div className="img w-56">
                    <img className='w-full h-full max-w-full max-h-full' src="https://upload.wikimedia.org/wikipedia/commons/thumb/6/62/CSS3_logo.svg/240px-CSS3_logo.svg.png" alt="" />

                </div>
            </div>
            <div className="item border p-5 flex flex-col items-center rounded-3xl  mx-2 mt-5">
                <div className="title text-text-color font-bold text-center text-3xl mb-5">
                    React.js
                </div>
                <div className="img w-56">
                    <img className='w-full h-full max-w-full max-h-full' src="https://upload.wikimedia.org/wikipedia/commons/thumb/a/a7/React-icon.svg/1200px-React-icon.svg.png" alt="" />

                </div>
            </div>
            <div className="item border p-5 flex flex-col items-center rounded-3xl  mx-2 mt-5">
                <div className="title text-text-color font-bold text-center text-3xl mb-5">
                    Tailwind
                </div>
                <div className="img w-56">
                    <img className='w-full h-full max-w-full max-h-full' src="https://avatars.githubusercontent.com/u/67109815?v=4&s=400" alt="" />

                </div>
            </div>
            <div className="item border p-5 flex flex-col items-center rounded-3xl  mx-2 mt-5">
                <div className="title text-text-color font-bold text-center text-3xl mb-5">
                    BootStrap
                </div>
                <div className="img w-56">
                    <img className='w-full h-full max-w-full max-h-full' src="https://user-images.githubusercontent.com/2327532/39481401-fa7d2992-4d30-11e8-886d-c4a3ee88147f.png" alt="" />

                </div>
            </div>
            <div className="item border p-5 flex flex-col items-center rounded-3xl  mx-2 mt-5">
                <div className="title text-text-color font-bold text-center text-3xl mb-5">
                    Responsive Design
                </div>
                <div className="img w-56">
                    <img className='w-full h-full max-w-full max-h-full' src="https://upload.wikimedia.org/wikipedia/commons/thumb/7/7f/Responsive_Web_Design_Logo.svg/1200px-Responsive_Web_Design_Logo.svg.png" alt="" />

                </div>
            </div>
        </div>
    )
}

export default Skils